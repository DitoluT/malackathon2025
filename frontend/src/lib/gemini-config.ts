...
export const getSystemPrompt = async (): Promise<string> => {
  try {
    // Cargar el prompt desde el archivo prompt.md
    const response = await fetch('/prompt.md');
    
    if (!response.ok) {
      console.warn('⚠️ No se pudo cargar prompt.md, usando prompt por defecto');
      return getDefaultPrompt();
    }
    
    const promptText = await response.text();
    console.log('✅ Prompt cargado desde prompt.md');
    console.log(promptText);
    return promptText;
    
  } catch (error) {
    console.error('❌ Error cargando prompt.md:', error);
    return getDefaultPrompt();
  }
};

// Prompt por defecto en caso de que no se pueda cargar el archivo
